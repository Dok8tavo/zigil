...
pub fn is(comptime T: type, comptime o: Options) z.Trait.Result {
    comptime {
        ...
        if (o.is_exhaustive) |is_exhaustive| if (is_exhaustive != info.is_exhaustive) return r.withFailure(.{
        ...
        for (o.with.strings) |name| if (!@hasField(T, name)) return r.withFailure(.{
            .@"error" = error.MissingValue,
            .expect = z.fmt("The enum type must have a value named \"{s}\".", .{name}),
            .option = z.fmt("has[{s}]", .{name}),
        });

        for (o.wout.strings) |name| if (@hasField(T, name)) return r.withFailure(.{
