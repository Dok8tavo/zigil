...
pub fn has(comptime T: type, comptime o: Options) z.Trait.Result {
    comptime {
        const r = z.Trait.Result.init(T, "has-size", "");
        ...
        switch (@typeInfo(T)) {
            ...
            .noreturn,
            .@"fn",
            => |_, tag| return if (o.allow_unknown) r else r.withFailure(.{
        ...
        if (o.bits.first) |min_bits| if (bits < min_bits) return r.withFailure(.{
        ...
        if (o.bits.last) |max_bits| if (max_bits < bits) return r.withFailure(.{
        ...
        if (o.bytes.first) |min_bytes| if (bytes < min_bytes) return r.withFailure(.{
        ...
        if (o.bytes.last) |max_bytes| if (max_bytes < bytes) return r.withFailure(.{
